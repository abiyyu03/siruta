...
var rtProfileRepository = new(RTProfileRepository)
var rtNotification = new(email.EmailRegistrationRepository)
...
func (r *RTProfileRegisterRepository) ApproveRegistrant(rtProfileId string) error {
	err := config.DB.Transaction(func(tx *gorm.DB) error {
		...
		rt, err := rtProfileRepository.FetchById(rtProfileId)

		if err != nil {
			return err
		}

		err = rtNotification.RtNotification(rt.RTEmail)
